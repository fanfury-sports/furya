...
import { ContinuousVestingAccount, Coins, MnemonicKey, MsgCreateVestingAccount } from "@fanfury-sports/feather.js";
import moment from "moment";
import { blockInclusion } from "../helpers/const";

describe("Auth Module (https://github.com/fanfury-sports/cosmos-sdk/tree/release/v0.47.x/x/auth)", () => {
    ...
    test('Must have vesting accounts created on genesis', async () => {
        try {
            ...
            expect(vestAccBalance[0])
                .toStrictEqual(Coins.fromString("990000000000ufury"));
        }
        catch (e) {
            expect(e).toBeUndefined();
        }
